...
## Standard Release Delivery Kit (Developed, Not Deployed)
...
- This delivery kit has not yet been built into final image tar files, deployed to `.31`, or exercised through upgrade/restore acceptance. Those are release operations, not implied by committing the scripts.
...
## Per-Node Credentials and Desired-State Reconciliation (Developed, Not Deployed)
...
- The formal release Compose no longer requires `TASKHUB_NODE_TOKEN`; existing rapid-development Compose retains it only for backward-compatible local Alpha operation. This increment has not been deployed to `.31`, built into images or pushed to either Git remote.
