...
class FairProjectBudget:
    ...
    def __init__(self, global_limit: int = 4, default_project_limit: int = 2):
        ...
        self.waiters: dict[str, deque[tuple[asyncio.Future, int]]] = defaultdict(deque)
        self.order: deque[str] = deque()
        self.last_project = ""
        self.lock = asyncio.Lock()

    @asynccontextmanager
    async def slot(self, project_id: str, project_limit: int | None = None):
        ...
        async with self.lock:
            ...
            self.waiters[project_id].append((future, limit))
            self._grant()
        await future
        try:
            yield
